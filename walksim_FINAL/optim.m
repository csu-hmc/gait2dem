function [result] = optim(input_problem)
	...
	% X0 = X0 + 0.01*(rand(size(X0))-0.5);		% add some randomness
	problem.X0 = X0;
	...
	for k=1:1
		problem.Jnnz = 1;
		% choose a random X but make sure the horizontal velocity is not too high (the logistic friction model saturates quickly and nonzeros are missed)
...
function [grad] = objgrad(X)
	...
	if (problem.Wtrack ~= 0)
		...
		hh = 1e-7;
		% GRF only depends only on 17 kinematic variables (q2..q18) in each node, and nodes are not coupled
...
function c = confun(X)
	...
	% pointers to constraints at node 1
	ic = 1:nstates;
	...
	for i=1:N
		...
		if (i < N)
			...
			x2 = X(vmx);							% use mirrored node 1 state
			x2(1) = x2(1) + problem.speed * X(end);	% add horizontal translation (speed*duration) for cycle
			u2 = X(nstates+vmu);					% use mirrored controls at node 1
		end
		
		if BE
			c(ic) = h*gait2dem(x2,u2) - (x2-x1);		
		else
			c(ic) = h*gait2dem((x1+x2)/2,u2) - (x2-x1);
		end
...
function J = conjac(X)
	...
	for k=0:1
		...
		for i=1:nvarpernode
			...
			for j=index
				J(:,j) = problem.Jpattern(:,j).*derivatives;
			end			% it may be worthwhile doing this without the loop, see http://www.frontiernet.net/~dmschwarz/genops.html
		end
	end
...
function report(X)
	...
	subplot(2,2,[1 3])
	x = reshape(X(1:end-1), problem.nvarpernode, problem.N);
	x = x(1:problem.nstates,:);			% use only the states
	drawstick(x);
